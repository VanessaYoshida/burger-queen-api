...
router.get('/', (req, res) => Orders.findAll({include: [{model: OrderProducts,
  include: [Products]}, Users]})
  .then(order => res.send(order))
);
router.get('/:id', (req, res) => {
  Orders.findByPk(req.params.id, {include: [{model: OrderProducts,
    include: [Products]}, Users]})
    .then(order => {
    ...
    .then((order) => {
      for (item of req.body.items) {
        Products.findOne({
          ...
          .then((product) => {
            OrderProducts.create({orderId: order.id,
